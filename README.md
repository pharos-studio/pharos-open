...
改端口：`PORT=3001 npm start`（Windows cmd 写成 `set PORT=3001 && npm start`）。

服务默认监听**所有网卡**（手机同 WiFi 访问靠的就是它），启动时会把可用的局域网地址打印出来。只想本机访问就设 `HOST=127.0.0.1`（Windows cmd 写成 `set HOST=127.0.0.1 && npm start`）。
...
首次运行拿到的是**空看板**（不含任何示例持仓），页面会提示你去「我的基金」加第一只。加基金先填**代码**：

1. 「我的基金」(`#/holdings`) → 快速添加基金 → 填 6 位**基金代码**
2. **名称 / 类型 / 跟踪指数 / 申购费 / 申购状态 / 实际上限会自动带出**。分类或指数只能启发式推断时，页面会要求确认
   - **类别**决定这只基金用哪套买卖纪律走。下拉里每一项都标了「适用于哪类基金」，选错了会算错，别跳过这段小字
   - **跟踪指数**是估值的尺子。自动识别不到时可以自己从下拉选或手填
3. 填买入金额 → 保存。五页立刻有内容

有两种情况**不会给买卖结论**——看板会明说，而不是给一个看着正常的错答案：

| 情况 | 页面表现 |
| --- | --- |
| 类别是「债券」或「现金」 | 标**待建设**：照常计入总资产与配置占比，但不参与买卖判定 |
| 拿不到跟踪指数（缺估值锚） | 标**缺估值锚·降级**：改用价格分位，理由里写明降级原因 |

> 想改分类显示名，或加自己的分类（比如把「主题·行业」叫成「我的医药」）→ 配置页底部的**类别管理**，新增时选一条内置算法绑定即可。
...
```bash
npm test          # 生成数据文件 → 11 组离线校验 → 脱敏审计，全程不联网
```
...
| `npm run test:offline` | 只跑 11 组离线校验（**需先 `npm run setup`**） |
| `npm run check:offline` | 证明这 11 组**真不联网**（给离线链加脚本前先跑它） |
...
- 你的数据全部在本地 `data/` 下，**不在 git 管理范围内**，升级不会碰它
- 若某次更新改了数据结构，程序会在**启动时自动处理**，处理前先自动备份一份到 `data/` 下的 `*.bak-时间戳` 文件 —— 你不需要做任何事，也不需要会备份
- 每次更新改了什么，见 [CHANGELOG.md](CHANGELOG.md)
...
- **我的基金**（路由仍为 `#/holdings`）—— 快速添加、自动档案、申购状态、用户每日上限、持仓与买入记录统一管理；新买入可按全局默认或逐笔选择积分抵扣。
  > 在途份额会在净值公布后**自动补填**（QDII 走 T+2），无需手动。
  > 份额公式：`金额 ÷ (1 + 有效费率) ÷ 成交净值`，保留 4 位。普通请求由服务端计算；只有显式券商真值会优先保留。
  > 升级到 v2 会先备份并全量重建旧份额与已有历史日期；任一费率或净值缺失时保持旧文件不变并进入只读模式，可在页面重试。
- **配置** —— 组合构成环形图（按策略线分块：宽基 / 红利·低波 / 主题·行业 / 商品·对冲 / 债券 / 现金，显示**当前实际占比**）+ 类别管理（可自建分类）+ 穿透分析·科技赛道透视（旭日图 + 基金画像卡，并如实标注「披露覆盖 X%」）。
- **复盘** —— 决策链路回放：每只基金拆成「信号理由 / 原始数据 / 估值信号表（含**上周**列）/ 结论（含上周判定对比）」四段，可折叠细看。
- **设置** —— 后端连接（地址 + API Key）。手机访问时必须在这里填电脑的局域网地址。

资产按**五套不同口径**判定（A股宽基 / 海外宽基 / 红利·低波 / 主题·行业 / 商品·对冲），因为「贵不贵」对不同资产要用不同的尺子。五套算法是**按资产类别**设计的通用规则，不是为某几只基金量身定做：
...
- **基金费率**（申购费 / 认购费 / 赎回费档位 / 管理费·托管费·销售服务费）：东方财富移动接口 `FundMNRateInfo`
  - ★ 申购费抓的是**销售渠道给的折后价**（这里是天天基金的 1 折），**不等于你在别的渠道实付的数** —— 银行常按原价收，券商与支付宝各不同。看板同时存了法定原价，鼠标悬停在「申购费」上可见
  - 运作费（管理 / 托管 / 销售服务费）**已从每日净值中扣除**，本看板只存档、不重算（重算就是重复扣）；赎回档位（如「持有 7 天内 1.5%，7 天后 0%」）目前也只存档、不参与计算
...
**关于本仓库的角色**：`pharos-open` 是**共享代码的唯一真相源** —— 公共部分（`backend/`、`public/`、`data/example/`、公开文档）一律改在这里。

维护者另有一份私有「数据机」仓库，只用来跑真实持仓数据、存放不公开的私人材料，两仓之间靠一份显式清单同步。⚠️ 对维护者的提醒：清单内的文件方向是**双向**的 —— 若漏掉「把本仓库同步回数据机」这一步就直接从数据机发布，本仓库里这些文件会被数据机上的旧版本覆盖，而且**不会报错**（`CONTRIBUTING.md` 第 6 节讲了完整顺序）。

作为贡献者你不需要关心这套机制：**公共代码改在本仓库、走 PR 就行。**

---

## 参与贡献

欢迎 issue 与 PR。动手前请读 [`CONTRIBUTING.md`](CONTRIBUTING.md)，里面有四条会直接影响 PR 能否合并的硬规矩：

- **`npm test` 必须全绿** —— 它会顺带跑**脱敏审计**：仓库里出现真实私人数据、本机路径、密钥，一律 FAIL
- **`docs/` 下的 `.md` 走白名单**，不是想加就能加（放错一个文件等于放行一份私人笔记）
- **同一个口径只允许一份实现** —— 改算法前先看那 5 个「唯一真相源」是哪几个
- **同一个文件不要两边各改一半**，要改就整份一次改完
