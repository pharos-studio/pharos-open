...
首次运行拿到的是**空看板**（不含任何示例持仓），五个页面都会提示你去「持仓」页加第一只。加基金只需要填**代码**：

1. 「持仓」页 → 添加基金 → 填 6 位**基金代码**
2. **名称 / 类别 / 跟踪指数 / 申购费会自动带出**（取自东方财富的基金档案与费率接口）。**申购费是只读的**，会跟着公告自动更新，界面上改不了（也无需你填）
...
| `npm run check:offline` | 证明那 9 组**真不联网**（给离线链加脚本前先跑它） |
