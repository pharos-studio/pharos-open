 ...
 *
 * ★★ 本脚本**需要联网**，因此**不进 `npm run test:offline`、也不进 CI**：
 *    CI 跑在境外 runner 上，连不上东财/蛋卷 → 断言失败 → 报出来的却是「数据不对」，
 *    真正原因（网络不通）看不出来。2026-09-18 首次 CI 就是被它搞红的。
 *    本机手动跑：`npm run test:network`。要往离线链里加脚本，先过断网探针验证。
 */
